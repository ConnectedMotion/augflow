# tests_individual_augmentations/test_translate.py
...
# Set up logging
log_filename = 'translate_augmentation_test.log'
...
# Test function
def test_translate_augmentation_edge_cases():
    """
    Test the TranslateAugmentation class with all configurations and edge cases for both detection and segmentation tasks.
    """
    # Create output directories
    output_dir = "translate_augmentation_test"
    os.makedirs(output_dir, exist_ok=True)
    ...
    for task in tasks:
        ...
        translate_configs = [
            ...
            #     'modes': ['non_targeted'],
            #     'config': {
            #         'translate_probability': 1.0,  # Ensure translation is applied
            #         'min_translate_x': -0.3,
            #         'max_translate_x': 0.3,
            #         'min_translate_y': -0.3,
            #         'max_translate_y': 0.3,
            #         'num_translations_per_image': 5,
            ...
            # # Both modes
            # {
            #     'modes': ['non_targeted', 'targeted'],
            #     'focus_categories': ['category3', 'category4'],
            #     'config': {
            #         'translate_probability': 1.0,
            #         'min_translate_x': -0.2,
            #         'max_translate_x': 0.2,
        ...
        for config_info in translate_configs:
            modes = config_info['modes']
            focus_categories = config_info.get('focus_categories', [])
            translate_config = config_info['config']
            augmenter = TranslateAugmentation(config=translate_config, task=task, modes=modes, focus_categories=focus_categories)
            augmented_dataset = augmenter.apply(dataset)
            
            # Assertions
            assert len(augmented_dataset.images) > 0, f"No images were augmented for translate in modes {modes} and task {task}."
            assert len(augmented_dataset.annotations) > 0, f"No annotations were augmented for translate in modes {modes} and task {task}."
            ...
            logging.info(f"Translate augmentation tests completed successfully for modes '{modes}' and task '{task}'.")
        
    logging.info("All translate augmentation tests completed successfully.")

# Run the test function
if __name__ == "__main__":
    test_translate_augmentation_edge_cases()
